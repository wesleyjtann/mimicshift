...
# ================== Modify ================== 
# Compute probability of occurence of a sentence
def seqOnlyPrepX(sentence, model, tokenizer, datasize, maxlen) :
    #Assigns indices to each request
    tok = tokenizer.texts_to_sequences([str(sentence)])[0]
...
def seqOnlyPrepY(sentence, model, tokenizer, datasize, maxlen) :
    #Assigns indices to each request
    tok = tokenizer.texts_to_sequences([str(sentence)])[0]
...
# ================== Modify ================== 
#Attack Scores
def calculateSequenceOnlyScores(modelP, modelQWithT, modelQWithoutT, tokenizer_normal, tokenizer_attack, df_normal, df_attack, df_test, maxlen):
    
    scoreDictionary = dict()
    count = 0
    #     df_temp = df_test[['Source IP','Dest IP', 'Input', 'Histo', 'Protocols', 'Info']].copy()
    # ===== Modify ===== 
    df_temp = df_test[['Source IP','Dest IP', 'Input']].copy()
    ...
    print('c1')
    # ===== Modify ===== 
    df_temp['Px'] = df_temp.apply(lambda x: seqOnlyPrepX(x["Input"], modelP, tokenizer_normal, norm_size, maxlen), axis=1)
    df_temp['Py'] = df_temp.apply(lambda x: seqOnlyPrepY(x["Input"], modelP, tokenizer_normal, norm_size, maxlen), axis=1)

    print('c2')
    # ===== Modify ===== 
    df_temp['Qx'] = df_temp.apply(lambda x: seqOnlyPrepX(x["Input"], modelP, tokenizer_attack, atk_size, maxlen),axis=1)
    df_temp['Qy'] = df_temp.apply(lambda x: seqOnlyPrepY(x["Input"], modelP, tokenizer_attack, atk_size, maxlen),axis=1)
    ...
    # add #No Scaling 
    # ===== Modify ===== 
    df_min_max = deepcopy(df_temp[['P_LSTM', 'QWithT_LSTM', 'QWithoutT_LSTM']]) #, 'QTilde']])
    df_min_max['Source IP'] = df_temp['Source IP']
    df_min_max['Dest IP'] = df_temp['Dest IP']

    # Computation of scores 
    df_min_max = df_min_max.rename(columns={0: 'P_LSTM', 1: 'QWithT_LSTM', 2: 'QWithoutT_LSTM'}) #, 5: 'QTilde'})
    df_min_max['P'] = (df_min_max['P_LSTM']) #+1).apply(lambda x: math.log(x))
    df_min_max['QWithT'] = (df_min_max['QWithT_LSTM']) #+1).apply(lambda x: math.log(x))
    df_min_max['QWithoutT'] = (df_min_max['QWithoutT_LSTM']) #+1).apply(lambda x: math.log(x))
    ...
    sr = df_min_max[['Source IP', 'Dest IP', 'P', 'PoverQ_online','PoverQ_offline', 'QWithT', 'QWithoutT']] #, 
...
def main():
    ...
    sr = calculateSequenceOnlyScores(modelP, modelQ_online, modelQ_offline, tokenizer_normal, tokenizer_attack, df_normal, df_attack, df_test, config['SEQUENCELENGTH'])
